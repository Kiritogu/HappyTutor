...
class AITutorStarter:
    ...
    def show_main_menu(self) -> str:
        ...
        print("1. 📝 Question Generator (Question)     - Generate questions based on knowledge base")
        print("2. 🔬 Deep Research (Research)     - Multi-round deep knowledge research")
        print("3. 🌐 Start Web Service (Web)       - Start frontend and backend services")
        print("4. ⚙️  System Settings (Settings)     - View/modify configuration")
        print("5. 🚪 Exit")
        logger.separator()

        while True:
            choice = input("\nPlease enter option (1-5): ").strip()
    ...
    async def run_research_mode(self):
        ...
        print("\n" + "=" * 70)
        print("🚀 Starting deep research (LangGraph)...")
        print("=" * 70)

        try:
            # Import LangGraph-based research pipeline
            ...
            # Extract research.* configs to top level
            ...
            preset_configs = {
                ...
                "standard": {
                    ...
                    "researching": {"max_iterations": 5, "iteration_mode": "fixed"},
                },
                "deep": {
            ...
            if preset in preset_configs:
                ...
                if preset == "auto":
                    print(
                        f"✅ Auto mode enabled (flexible iterations)"
                    )
                else:
            ...
            research_id = f"research_{timestamp}"

            # Build and run LangGraph
            graph = build_research_graph(enable_checkpoint=False)
            ...
            async def progress_callback(event):
                ...
                if stage == "planning":
                    if status == "decomposed":
                        print(f"   ✅ Decomposed into {event.get('subtopics_count', 0)} subtopics")
                    elif status == "queue_ready":
                        print(f"   ✅ Queue initialized with {event.get('total_blocks', 0)} blocks")
                elif stage == "researching":
                    if status == "block_completed":
                        print(f"   ✅ Completed: {event.get('block_id', '')} ({event.get('iterations', 0)} iterations)")
                    elif status == "researching_block":
                        print(f"   📖 Researching: {event.get('block_id', '')}")
                elif stage == "reporting":
                    if status == "report_ready":
                        print(f"   ✅ Report generated ({event.get('word_count', 0)} words)")
            ...
            print("=" * 70)
            print(f"📁 Report location: {final_report_path}")
