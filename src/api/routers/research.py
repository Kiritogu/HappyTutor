...
async def optimize_topic(
    ...
    try:
        ...
        # Init Agent
        agent = RephraseAgent(config=config)
...
async def websocket_research_run(websocket: WebSocket):
    ...
    try:
        ...
        # Legacy support (ignored, using LangGraph only)
        preset = data.get("preset")  # For backward compatibility
        research_mode = data.get("research_mode")  # Ignored
        ...
        graph = build_research_graph(enable_checkpoint=True)

        # WebSocket callback for progress
        ...
        research_id = result.get("research_id", task_id)
        ...
        try:
            ...
            # Result was already computed during graph.ainvoke above
