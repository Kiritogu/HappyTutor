...
Architecture:
    ...
    ┌─────────┼─────────┐
    ↓         ↓         ↓
LangChain  CloudProvider  LocalProvider
...
Usage:
    ...
    # LangChain provider (direct access)
    from src.services.llm import LangChainProvider
    response = await LangChainProvider.complete("Hello!", binding="openai")
"""
...
def __getattr__(name: str):
    ...
    if name == "LangChainProvider":
        try:
            ...
            raise ImportError(
                "LangChain packages not installed. Run: pip install langchain-openai langchain-community"
            )
