...
Architecture:
    ...
    ┌─────────┴─────────────────────┐
    ↓                               ↓
LangChainProvider          CloudProvider/LocalProvider
(langchain_provider)       (cloud_provider/local_provider)
    ↓                               ↓
LangChain SDK              OpenAI/DeepSeek/Ollama/etc

Provider Selection:
- LangChain (default): use_langchain=true in config/env
- Legacy: use_langchain=false falls back to original cloud/local providers

Routing (for legacy providers):
...
_langchain_provider = None
_langchain_checked = False


def _get_langchain_provider():
    """
    Get the LangChain provider module (lazy import).

    Returns:
        LangChainProvider class or None if not available
    """
    global _langchain_provider, _langchain_checked
    ...
    try:
        ...
        _langchain_provider = LangChainProvider
        logger.debug("LangChain provider loaded successfully")
    except ImportError as e:
        logger.debug(f"LangChain provider not available: {e}")
        _langchain_provider = None
    except Exception as e:
        logger.warning(f"Failed to load LangChain provider: {e}")
        _langchain_provider = None

    return _langchain_provider


def _should_use_langchain(config) -> bool:
    """
    Determine if LangChain provider should be used.

    Args:
        config: LLMConfig instance

    Returns:
        True if LangChain should be used
    """
    # Check config setting
    use_langchain = getattr(config, "use_langchain", True)
...
async def complete(
    ...
    **kwargs,
    ...
    Routes to LangChain provider (if enabled) or legacy cloud/local providers.
    ...
    if _should_use_langchain(config):
        try:
            provider = _get_langchain_provider()
            if provider:
            ...
            # Log and fall back to legacy provider
            logger.warning(f"LangChain provider failed, falling back to legacy: {e}")

    # Legacy provider path (cloud/local)
...
async def _legacy_complete(
    ...
    **kwargs,
    ...
    """
    Legacy LLM completion using cloud/local providers.

    This is the original implementation before LangChain integration.
    Used as fallback when LangChain is disabled or unavailable.
    """
    ...
    **kwargs,
    ...
    if _should_use_langchain(config):
        try:
            ...
            # Log and fall back to legacy provider
            logger.warning(f"LangChain stream failed, falling back to legacy: {e}")
...
async def _legacy_stream(
    ...
    **kwargs,
    ...
    """
    Legacy LLM streaming using cloud/local providers.
