...
export const ResearchDashboard: React.FC<ResearchDashboardProps> = ({
  ...
  const stepTabsContent = (
    <div className="flex items-center gap-1 bg-white/70 dark:bg-slate-900/50 backdrop-blur-xl p-1 rounded-xl border border-slate-200/60 dark:border-slate-800/60 shadow-sm">
      {steps.map((step, idx) => {
        ...
        return (
          <button
            ...
            className={`
              ...
              ${
                isSelected
                  ? "bg-white dark:bg-slate-800 shadow-sm border border-slate-200 dark:border-slate-700 text-emerald-700 dark:text-emerald-300"
                  : available
    ...
    <div className="flex-1 flex items-center justify-center">
      ...
      >
        <div
          className={`w-16 h-16 rounded-full flex items-center justify-center mb-4 ${
            isReportingActive
              ? "bg-emerald-50 dark:bg-emerald-900/35"
              : isCompleted
        ...
        >
          {isCompleted ? (
            ...
            <PenTool
              className={`w-8 h-8 ${isReportingActive ? "text-emerald-600 dark:text-emerald-400" : "text-slate-400 dark:text-slate-500"}`}
            />
          )}
        </div>

        {isReportingActive && (
          <Loader2 className="w-6 h-6 text-emerald-600 dark:text-emerald-400 animate-spin mb-4" />
        )}
        {isCompleted && (
          <CheckCircle2 className="w-6 h-6 text-emerald-500 dark:text-emerald-400 mb-4" />
        )}
        ...
        {isReportingActive && reporting.currentSection && (
          <div className="bg-emerald-50 dark:bg-emerald-900/25 border border-emerald-100 dark:border-emerald-900/40 rounded-lg px-4 py-2 mb-4 w-full">
            <p className="text-xs text-emerald-700 dark:text-emerald-300 font-medium uppercase tracking-wider mb-1">
              {t("Currently Writing")}
            </p>
        ...
        {isReportingActive &&
          ...
          reporting.sectionIndex !== undefined && (
            <div className="w-full mb-4">
              <div className="h-2 bg-emerald-100 dark:bg-emerald-900/35 rounded-full overflow-hidden">
                <div
        ...
        {isCompleted && reporting.generatedReport && (
          <button
            ...
            className="mt-6 flex items-center justify-center gap-2 w-full py-3 px-4 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl font-medium transition-colors shadow-sm shadow-emerald-500/20"
          >
  ...
  return (
    <div className="flex flex-col h-full bg-transparent">
      ...
      <div className="bg-white/70 dark:bg-slate-950/20 border-b border-slate-200/60 dark:border-slate-800/60 px-6 py-4 flex items-center justify-between shadow-sm shrink-0 z-20 relative backdrop-blur-xl">
            ...
            className={`p-2 rounded-lg ${
              global.stage === "idle"
                ...
                : global.stage === "completed"
                  ...
                  : "bg-emerald-100 dark:bg-emerald-900/35 text-emerald-700 dark:text-emerald-300"
            }`}
          >
            {isCompleted ? (
        ...
        {/* View Switcher */}
        <div className="flex bg-white/70 dark:bg-slate-950/20 p-1 rounded-lg border border-slate-200/60 dark:border-slate-800/60 backdrop-blur-xl">
          <button
            onClick={() => setActiveView("process")}
            className={`flex items-center gap-2 px-4 py-1.5 rounded-md text-sm font-medium transition-all ${
              activeView === "process"
                ? "bg-white dark:bg-slate-900 text-emerald-700 dark:text-emerald-300 shadow-sm border border-slate-100 dark:border-slate-800"
      ...
      {activeView === "process" ? (
        ...
        <div className="flex-1 flex flex-col min-h-0 overflow-hidden">
          {/* Report Toolbar */}
          <div className="px-6 py-3 border-b border-slate-200/60 dark:border-slate-800/60 flex justify-end items-center bg-white/70 dark:bg-slate-950/20 backdrop-blur-xl shrink-0">
            <div className="flex gap-2">
              {onAddToNotebook && (
                <button
                  onClick={onAddToNotebook}
                  className="text-emerald-700 dark:text-emerald-300 hover:text-emerald-800 dark:hover:text-emerald-200 flex items-center gap-1.5 text-sm px-3 py-1.5 rounded-lg hover:bg-emerald-50 dark:hover:bg-emerald-900/20 transition-all"
                >
                ...
                <article className="prose prose-slate dark:prose-invert max-w-none prose-headings:text-slate-800 dark:prose-headings:text-slate-100 prose-p:text-slate-600 dark:prose-p:text-slate-300 prose-a:text-emerald-700 dark:prose-a:text-emerald-400 prose-img:rounded-xl prose-table:border-collapse prose-th:border prose-th:border-slate-300 dark:prose-th:border-slate-600 prose-th:bg-slate-50 dark:prose-th:bg-slate-700 prose-th:p-2 prose-td:border prose-td:border-slate-200 dark:prose-td:border-slate-600 prose-td:p-2">
                  ...
                  <ReactMarkdown
                    ...
                    components={{
                      ...
                      h2: ({ node, ...props }) => (
                        <h2
                          className="text-2xl font-bold mt-8 mb-4 text-emerald-900 dark:text-emerald-300"
                          {...props}
                        />
                      ),
                      h3: ({ node, ...props }) => (
                      ...
                      blockquote: ({ node, ...props }) => (
                        <blockquote
                          className="border-l-4 border-emerald-300 dark:border-emerald-700 pl-4 py-2 my-4 bg-emerald-50/50 dark:bg-emerald-900/20 text-slate-600 dark:text-slate-300 italic"
                          {...props}
                        />
                      ),
                      a: ({ node, href, ...props }) => {
                        ...
                        return (
                          <a
                            href={href}
                            onClick={handleClick}
                            className="text-emerald-700 dark:text-emerald-400 hover:text-emerald-900 dark:hover:text-emerald-300 underline decoration-emerald-300 dark:decoration-emerald-700 hover:decoration-emerald-500 dark:hover:decoration-emerald-500"
                            {...props}
                          />
                        );
                      },
